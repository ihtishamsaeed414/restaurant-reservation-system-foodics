...
const fetchBranches = async () => {
  ...
  try {
    ...
    branches.value = response.data.data.filter(branch => !branch.accepts_reservations);
  } catch (error) {
...
const save = async () => {
  if (selectedBranch.value) {
    try {
      isLoading.value = true;
      await axios.put(`/api/branches/${selectedBranch.value}`, {
      ...
      // Emit save event with the selected branch
      emit("save", selectedBranch.value);
      
      // Refresh the branches list to remove the updated branch
      await fetchBranches();
